...
class DigestFreshener(unittest.TestCase):
    ...
    def test_a_value_nothing_reads_comes_back_as_a_question_not_a_rewrite(self):
        """It used to be cleared here and filed to a log nobody opens, in whichever
        session happened to run next. The card keeps it; the desk that set it hears."""
        with tempfile.TemporaryDirectory() as d:
            cfg, bdir = _proj(d)
            cardlib.new_card(bdir, "MOBILE", task_id="1", priority="low")
            out = sbd.run({"cwd": d}, None)
            self.assertIn("#1", out)
            self.assertIn("low", out)
            self.assertEqual(cardlib.load(bdir)[0]["priority"], "low")   # not touched
            self.assertIsNone(sbd.run({"cwd": d}, None))                 # said once

    def test_setting_a_real_level_ends_it_and_re_arms(self):
        with tempfile.TemporaryDirectory() as d:
            cfg, bdir = _proj(d)
            card = cardlib.new_card(bdir, "MOBILE", task_id="1", priority="low")
            self.assertIsNotNone(sbd.run({"cwd": d}, None))
            cardlib.set_fields(card, priority="P3")
            self.assertIsNone(sbd.run({"cwd": d}, None))
            cardlib.set_fields(cardlib.load(bdir)[0], priority="urgent-ish")
            self.assertIsNotNone(sbd.run({"cwd": d}, None))
