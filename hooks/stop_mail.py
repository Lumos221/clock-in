...
def sweep(mail_dir, office):
    ...
    invisible to every addressee forever (a commissioned dept report file-dropped as
    plain markdown sat with empty columns and no nudge), so the CEO office gets told
    instead of nobody."""
...
def run(data, text):
    ...
    if data.get("stop_hook_active"):
        return None
    # Office-to-office channel: a teammate is not an office, and unmarked it reads as
    # the CEO — so it would be nudged about mail it does not own (2026-07-27).
    ...
    if mine:
        ...
        parts.append("%d unread for %s — %s. Read each (%s/), act or reply (a reply is "
                     "a NEW note: from/to/re/time/status: unread), then flip its "
                     "`status: read`."
                     % (len(mine), office, lst, os.path.relpath(mail_dir, root)))
    if dead:
        parts.append("%d DEAD letter(s) with no to:/status: frontmatter (%s) — "
                     "invisible to every addressee. Add the frontmatter, or move the "
                     "file out: this box is for ADDRESSED inter-office notes, dept "
                     "reports go via SendMessage."
