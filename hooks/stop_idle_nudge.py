...
stamp every JSONL line with agentName / agentSetting / teamName. The lead ("team-lead"), the
...
# nudged for "unreported work" (Marketing report → `git log`
...
NUDGE = ('You are going idle with unreported work. Send your 4-line report now — '
         'SendMessage(to:"team-lead", summary:"…", message:"…"). Plain text in your '
         'pane is invisible to the CEO. Blocked → report blocked instead.')


def identity(transcript_path):
    """(agentName, agentSetting, teamName) — delegates to the ONE reader in hooklib.
    Kept as a name because callers already import it from here."""
    return hooklib.session_agent(transcript_path)
