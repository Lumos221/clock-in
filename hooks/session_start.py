...
Same sentinel pattern over DECISIONS.md (field causes, a live project 07-14): a tagged
...
def decisions_flags(root, cfg):
    ...
    if no_impl:
        flags.append(
            "⚠ %d recent DECISIONS entr%s no **Impl:** line (%s) — add its card (`#<id>`), "
            "an explicit `parked: <why>`, or `none-needed`. A ruling queued only in prose "
            "is silent loss: nothing tracks it."
    ...
    if board is not None:
        try:
            ...
            if live:
                ...
                parts.append(
                    "⚠ %d Active card(s) carry no platform task_id (%s). Register each via "
                    "TaskCreate — hand-only cards rot. Not loaded → ToolSearch "
                    "select:TaskCreate,TaskUpdate,TaskList,TaskGet. Genuinely absent "
                    "(widget-gated session) → spawn the 书记处 Registrar and route the "
                    "lifecycle through it (reference/task-widget.md)."
...
def detach_stale_ids(root, cfg, data):
    ...
    the CEO never journals migration state into card headings (field cause, a live project
    ...
    if not sid:
        return 0
    # A store we cannot FIND must never license a sweep. This routine reads absence as
    # proof that an id died, so an unresolved store dir reads as "every id died" and
    # strips task_id off the whole board in one pass. An EMPTY store is fine — that is
    # a genuinely fresh lead before its first TaskCreate, exactly the case this exists
    # for. Missing is not empty. (The key does not follow a resume — hooklib.team_key.)
    if hooklib.local_office(data.get("cwd") or ""):
        return 0            # a 分公司 must never sweep the CEO board with its own store
    store = hooklib.tasks_dir(sid, data.get("cwd") or "")
    ...
    for card in cardlib.load(bdir):
        ...
        if tid.isdigit() and not os.path.exists(os.path.join(store, "%s.json" % tid)):
            try:
                ...
                pass
    # Retired cards used to keep their platform id, and the platform reissues those ids
    # every session — so a live card and a card retired in June both answer to `115`, and
    # every id-keyed path picks whichever it reaches first. Retirement surrenders the id
    # now (cardlib.retire); this heals the stores that predate that. Only COLLIDING ones
    # are touched: an id worn by exactly one retired card is inert, and rewriting hundreds
    # of settled files to tidy it would be churn for nothing.
    live = {cardlib.clean(c.get("task_id", "")) for c in cardlib.load(bdir)}
    live = {t for t in live if t.isdigit()}
    for sub in ("done", "archive"):
        for card in cardlib.load(bdir, sub):
            tid = cardlib.clean(card.get("task_id", ""))
            if tid.isdigit() and tid in live:
                try:
                    cardlib.set_fields(card, task_id=cardlib.EMPTY)
                    n += 1
                except Exception:
...
def briefs_autopatch(root, cfg):
    """Frontmatter auto-migration, so a schema change never costs a re-recruit. For each
    roster brief, ADD any template frontmatter field the brief lacks (template's literal line, e.g. `model: sonnet`); NEVER overwrite a present
    ...
    for h in handles:
        ...
        # registers "note" as a tool name; field case 2026-07-19: every a live project
...
def pane_flags(root, data):
    ...
    busy-flag (responsive Registrar at isActive:false), so
    ...
    if not sid:
        return []
    team = hooklib.team_config(sid, data.get("cwd") or "") if hooklib else None
    if not team:
        return []
    tasks_dir = hooklib.tasks_dir(sid, data.get("cwd") or "")
    if not tasks_dir:
        return []
    try:
...
def main():
    ...
    # line. Depts get the slim brief; the Registrar (a
