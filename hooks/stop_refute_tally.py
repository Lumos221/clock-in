...
def tally(root, thresholds, roster=None):
    ...
    # the circuit breaker. The Auditor's
    ...
    if known:
        # The CEO's own cards (specs, plans, design docs — doctrine's "you diagnose
        # and spec") gate through L2 like everyone's and file markers under CEO, which
        # sits in no roster and has no agent file. A deliberate self-produced card is
        # not an alias; its bounces bucket cleanly under CEO.<id>, so the breaker
        # still counts them. Added HERE, past the emptiness gate — baked into
        # _known_handles it would arm the detector on rosterless projects.
        known.add("ceo")
        for dkey, (raw, n) in prefixes.items():
            key = "alias-%s" % dkey
            if dkey not in known:
                _flag_once(root, key, "督察",
                           "审查 ledger holds a non-roster handle '%s' (%d file(s)) — an alias splits per-task counts and evades the circuit breaker. Normalize to the roster handle." % (raw, n))
            else:
                _unflag(root, key)
