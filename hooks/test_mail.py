...
class EveryNudgeReachesTheAgent(unittest.TestCase):
    """The dispatcher kept only the FIRST module that returned a line and dropped the
    rest — while each of those modules stamps its one-nudge-per-state file BEFORE it
    returns. A dropped line was therefore a nudge spent and never re-armed: unread mail
    stayed unannounced behind a capacity line that happened to run three places earlier
    in the chain (2026-08-04, "最新的几条mail怎么都没读啊，也没见到有stophook提示要读")."""

    def _dispatch(self, speaks):
        import types, io, json as _json, importlib.util
        here = os.path.dirname(os.path.abspath(__file__))
        spec = importlib.util.spec_from_file_location("sd", os.path.join(here, "stop_dispatch.py"))
        sd = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(sd)

        def fake_load(name):
            m = types.ModuleType(name)
            m.run = lambda data, text, _n=name: speaks.get(_n)
            if name == "hooklib":
                m.last_assistant_text = lambda p: ""
            return m
        sd._load = fake_load
        old_in, old_err = sys.stdin, sys.stderr
        sys.stdin = io.StringIO(_json.dumps({"hook_event_name": "Stop",
                                             "transcript_path": "", "cwd": "."}))
    ...
    def test_a_later_module_is_not_swallowed_by_an_earlier_one(self):
        code, out = self._dispatch({"stop_capacity": "CAPACITY", "stop_mail": "MAIL"})
        self.assertEqual(code, 2)
        self.assertIn("CAPACITY", out)
        self.assertIn("MAIL", out, "the mail nudge was consumed and never delivered")

    def test_identical_lines_are_said_once(self):
        code, out = self._dispatch({"stop_boss_board": "SAME", "stop_mail": "SAME"})
        self.assertEqual(out.count("SAME"), 1)

    def test_a_silent_chain_still_lets_the_turn_end(self):
        code, out = self._dispatch({})
        self.assertEqual(code, 0)
        self.assertEqual(out, "")

    def test_the_mail_nudge_alone_still_blocks(self):
        code, out = self._dispatch({"stop_mail": "📮 mail: 3 unread"})
        self.assertEqual(code, 2)
        self.assertIn("📮 mail: 3 unread", out)
