...
class L2PerTask(unittest.TestCase):
    ...
    def test_alias_handle_is_surfaced_not_silently_bucketed(self):
        """Regression (a live project, web.40.1.fail): a legacy alias splits one task's
    ...
    def test_agent_file_handles_count_as_known(self):
        """Field report (a live project): on-demand workers (Fin, Prof_/Spec_) live outside
    ...
    def test_the_ceos_own_markers_are_deliberate_not_an_alias(self):
        """The CEO's own cards (specs, plans, design docs) gate through L2 and file
        markers under CEO, which is in no roster and has no agent file. A deliberate
        self-produced card is not an alias — and the circuit breaker still counts it."""
        with tempfile.TemporaryDirectory() as d:
            _touch(os.path.join(_reviews(d), "CEO.316.1.fail"))
            _touch(os.path.join(_reviews(d), "web.40.1.fail"))       # still an alias
            tally_mod.tally(d, TH, roster=["Frontend"])
            flags = [e for e in _open_items(d, "督察") if "alias" in e["text"]]
            self.assertEqual(len(flags), 1)
        ...
        with tempfile.TemporaryDirectory() as d:
            _touch(os.path.join(_reviews(d), "CEO.316.1.fail"))
            _touch(os.path.join(_reviews(d), "CEO.316.2.fail"))
            tally_mod.tally(d, TH, roster=["Frontend"])
            self.assertEqual(len(_open_items(d, "CEO")), 1)          # breaker still fires
