...
gap mechanically at every turn end: one mtime sweep (no parsing), regen only when
some card is newer than the digest. Zero tokens, never blocks, fail-open; inert
...
def run(data, text):
    ...
    try:
        ...
        return None
    try:
        if os.path.isdir(cardlib.board_dir(root, cfg)) and cardlib.digest_stale(root, cfg):
