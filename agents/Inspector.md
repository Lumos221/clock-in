...
(it's the auto-discovery key). **Pin `model:` by prefix — `Prof_` → `opus`, `Spec_` → `sonnet`**
(`orchestrate/reference/model-routing.md`). Underperforming expert → retune its file (lighter touch).
