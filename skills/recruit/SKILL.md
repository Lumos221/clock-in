...
2. **Always include the three standing files** — copied **verbatim** (project-independent: no owned files, no customization, **not** in `roster`):
   ...
   - **书记处** (Registrar — mechanical task-lifecycle proxy for widget-gated sessions; see `orchestrate/reference/task-widget.md`): `orchestrate/templates/registrar.md` → `.claude/agents/Registrar.md`. The CEO spawns it only when its own session lacks the task tools.
