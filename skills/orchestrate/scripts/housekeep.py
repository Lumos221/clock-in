...
def candidates(root, cfg, now=None):
    """[(abs_path, rel_path, size)] of stale, unreferenced files across configured
    dirs. archive/ subtrees are skipped; referenced files are protected."""
...
def archive(root, cfg, now=None):
    ...
    moved = []
    dirs = {os.path.abspath(d): (d, rel) for d, rel, _ in hk_dirs(root, cfg)}
    for p, relp, size in candidates(root, cfg, now):
        base = next((d for d in dirs if os.path.abspath(p).startswith(d + os.sep)), None)
        if base is None:
            continue
...
def prune(root, cfg, days, now=None):
    ...
    cutoff = now - days * 86400
    gone = []
    for d, rel, _ in hk_dirs(root, cfg):
...
def main(argv):
    ...
    if "--days" in argv:
        try:
            ...
            sys.stderr.write("usage: orchestrate-housekeep prune --days <N>\n")
            return 1
    ...
    if cmd == "prune":
        if days is None:
            ...
            return 1
        gone = prune(root, cfg, days)
        print("housekeep: pruned %d archived file(s) older than %d days." % (len(gone), days))
        return 0
    sys.stderr.write("usage: orchestrate-housekeep scan|run|prune --days <N>\n")
    return 1
