...
- **Depts — sonnet by the brief's pin, no per-spawn param needed (0.9.26):** the dept brief's frontmatter pins `model: sonnet` (the template default — Boss's ruling 2026-07-19 after the per-spawn discipline failed three times in the field), so a param-less spawn comes up sonnet mechanically. **A Boss-designated tier lives in that same frontmatter** (e.g. Marketing pinned `model: fable` at recruit); an explicit `model:` on the spawn call still overrides for a one-off. The spawn guard now blocks a param-less spawn only when the brief carries NO pin (a pre-0.9.26 brief — run the /recruit upgrade pass); the session-start line still arms this regime mechanically when the harness reports the model. With piece-level specs the head's planning job is gone — sonnet executes, reviews staff (haiku for script-shaped pieces, the model-routing test unchanged), invokes L2, reports evidence.
