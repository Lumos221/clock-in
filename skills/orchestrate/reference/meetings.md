...
For an overnight run: the CEO **fills a few fields** (it authors the content) and the script renders a clean PDF/PNG, **auto-opened** when the Boss asks:

`echo '{"shipped":[…],"queued":[…],"needs_boss":[…],"note":"…"}' | orchestrate-brief --pdf` (`--png`, or omit for HTML). The input it takes:
