...
- **External terminal seats are not 部门**: a 子公司 / 外聘 seat is an executing terminal for
  existing 部门 (`docs/board/seats/`), never in `roster`, never given a dept brief;
  dispatch via `reference/external-seats.md`.
...
**nickname（花名，显示名，不是 handle）：** 每次 dept spawn，CEO 起一个花名写进 spawn `description`：`nickname=Lisa`（与 `effort=high` 同款 `=` 分隔符；中文键 `花名=阿岚` 也认，`:` 兼容但模板用 `=`）。**名字必须由 CEO 起，不能交给代码随机分配**：Boss 用花名点名（「让 Lisa 改」），起名的人才知道她在说谁；随机是你起名时的自由，不是机器的。Boss Board 任务卡与部门卡显示 `Frontend · Lisa`；handle 仍是 `<dept>-<卡号>`——数字后缀是容量/停滞检查还原部门的依据，不可换成词。同部门内不重名，与外部 contractor 席位共用人名池。
