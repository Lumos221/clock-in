...
**Orchestration files are off-limits to every dept** (no carve-out list to forget): `docs/SoT.md`, `docs/TaskBoard.md`, `docs/BACKLOG.md`, `docs/DECISIONS.md`, `docs/reviews/`, `docs/复盘-*.md`, `docs/handover-*.md`. These are owned by the **CEO / 审查官 / 人事部** per their roles — a 部门 only edits **its own task card's `status`** on `TaskBoard.md`, nothing else there.

**Canonical file — which output "matters" (earns a `SoT.md` pointer):** a dept often produces several rounds (Law: 3 research passes; Marketing: 5 competitor studies). The **canonical** one = the current authoritative answer to a question the project acts on, superseding the rounds. Test: *would an outsider redo work, or lose a depended-on conclusion, if it vanished?* → yes.
**How it's set:** the **dept proposes** its canonical file **in its report** (e.g. "canonical: `docs/合规/红线法律依据.md`"); the **CEO gates** it — accept / send back to refine / bounce — and only then points `SoT.md` at it (the **Boss** owns project-defining ones). "Authoritative" = **passed that gate**, not self-declared. **Not frozen:** the pointer always names the *current* best and re-points as the dept supersedes; if it isn't good enough yet, it simply isn't canonical yet. One pointer per answered question; not every dept has one.
