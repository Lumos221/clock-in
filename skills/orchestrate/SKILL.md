...
|---|---|---|---|
| `docs/SoT.md` | **source of truth** — the project's CLAUDE.md: goal · Now (3 capped slots) · curated pointers. **Hard cap ~15 lines** (it's injected every session); decisions live in `CANON.md`/`DECISIONS.md`, never restated here | **CEO** (authoring.md-grade) | **each session** (it's small) |
| `docs/TaskBoard.md` | **live board** — active cards (todo/doing/review/blocked) + a machine-kept *Recently shipped* tail (~5) | CEO writes cards; depts own their `status`; **CEO marks done on an L2 pass** + deletes the card — the **completion hook** records it under *Recently shipped* | while orchestrating |
