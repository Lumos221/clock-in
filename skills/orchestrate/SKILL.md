...
- **报告即停:** done → a 部门 commits, reports, then **STOPs** — it never starts the next leg unprompted. Its report **proposes its domain's next-steps**; the CEO sequences across domains.
...
A **standing file** (`.claude/agents/Inspector.md`), invoked **one-shot, no `name`** — independence = fresh instances + `@BOSS[Inspector]` verdicts the CEO can't filter. Full detail → `reference/inspector.md`. The CEO-side loop:
