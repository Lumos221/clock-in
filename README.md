...
<p align="center">
  <img src="docs/assets/boss-board.png" width="820"
       alt="The Boss Board dashboard as a message list: a rail of departments split into the ones that need a decision and the ones with unread updates, the open conversation as a thread of dated messages with an ask flagged NEEDS YOU, and one composer at the bottom addressed to that department.">
</p>
<p align="center"><sub>The Boss Board reads like a chat client: one conversation per department, unread counts on the rail, and a thread of dated messages. An ask wears <em>needs you</em> until you answer it, then <em>answered</em>, with your reply quoting it. You answer from the composer and the answer goes back to the session that raised it. Every file path on it is a link.</sub></p>

<p align="center">
  <img src="docs/assets/boss-board-tasks.png" width="820"
       alt="The Tasks view: every card drawn on the pipeline it is actually in, from unassigned through dispatched, in progress, review and done, with the department, the priority and how long it has sat in its current stage.">
</p>
<p align="center"><sub>Every card wears the pipeline it is actually in, so work stuck at the review gate is the loudest thing on the page. The clock on each card is time <em>in this stage</em>, not time since someone last touched the file.</sub></p>
...
- **Spends the smartest model on judgment, cheap models on typing.** The manager diagnoses and writes the spec; department heads work to that spec a tier below it; the staff they hire run cheaper still, down to the tier that just applies a diff. **Your most expensive tier is never spent on your behalf** — routing anything to it takes your explicit go-ahead, every single time, and the request comes to you with a fallback already named.
...
## The manager thinks; the departments type

The session you start is the brain, and it stays the brain. It reads back what you meant, diagnoses from your marked screenshots without opening a single file, and hands each department a written spec: what to build, what not to touch, what is fixed and what is theirs to choose, and what evidence the reviewer will want. It holds words, tables and reports — never code.

That split is what makes cheaper specialists safe to use. A department given a goal and a free hand goes wrong in ways nobody sees until you do; the same department given a spec and a reviewer does the work you actually asked for, at a fraction of the cost of thinking at the top tier.
